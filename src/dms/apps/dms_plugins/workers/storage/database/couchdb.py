...
class DatabaseManager(object):
    """
    Handles CouchDB "document" (metadata) Management.
    """
    def get_DB_document(self, document):
        print "Getting Document from DB", document

    def store_DB_documnet(self, document):
        print "Setoring Document into DB", document

    def delete_DB_document(self, document):
...
class CouchDBMetadata(object):
    ...
    """
    def __init__(self):
#        self.filesystem = DatabaseManager()
        self.database = DatabaseManager()

    def store(self, request, document):
        # storing no metadata for no doc rule documents
        if document.get_docrule().no_doccode:
            return document
        database = self.database.store_DB_documnet(document)
        return document

    def update_metadata_after_removal(self, request, document):
        self.database.delete_DB_document(document)
...
#            pass # our directory with all metadata has just been deleted %)
    ...
    def retrieve(self, request, document):
        self.database.get_DB_document(document)
        return document
#        directory = self.filesystem.get_or_create_document_directory(document)
#        if document.get_docrule().no_doccode:
#            revision = 'N/A'
...
class CouchDBMetadataStoragePlugin(Plugin, BeforeStoragePluginPoint):
