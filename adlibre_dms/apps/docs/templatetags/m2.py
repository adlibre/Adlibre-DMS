"""Markdown2 filter, requires the python-markdown2 library
from http://code.google.com/p/python-markdown2
...
In any template do:
    ...
    {% load m2 %}
    {% filter md2:"safe, code-friendly, code-color"  %}
    {{ variable}}
    {% endfilter %}
    ...
    {% load m2 %}
    {{ variable|md2:"safe, code-friendly, code-color: noclasses|linenos" }}
...
def md2(value, arg=''):
    ...
    try:
        ...
        if settings.DEBUG:
            raise template.TemplateSyntaxError, "Error in {% md2 %} filter: python-markdown2 isn't installed."
